...
func GetUser(name string, config Config) *model.SimpleUser {
	...
	if config.Account {
		user.Account = model.SimpleAccount{Number: name + "_account"}
	}

	for i := 0; i < config.Pets; i++ {
		user.Pets = append(user.Pets, &model.SimplePet{Name: name + "_pet_" + strconv.Itoa(i+1)})
	}

	for i := 0; i < config.Toys; i++ {
		user.Toys = append(user.Toys, model.SimpleToy{Name: name + "_toy_" + strconv.Itoa(i+1)})
	}

	if config.Company {
		user.Company = model.SimpleCompany{Name: "company-" + name}
	}
	...
	for i := 0; i < config.Languages; i++ {
		...
		language := model.SimpleLanguage{Code: name, Name: name}
	...
	if config.NamedPet {
		user.NamedPet = &model.SimplePet{Name: name + "_namepet"}
	}

	return &user
}

func CheckSimplePet(t *testing.T, pet model.SimplePet, expect model.SimplePet) {
	if pet.ID != 0 {
		var newPet model.SimplePet
		if err := DB.Model(&model.Pet{}).Where("id = ?", pet.ID).First(&newPet).Error; err != nil {
...
func CheckPet(t *testing.T, pet model.Pet, expect model.SimplePet) {
	if pet.ID != 0 {
		var newPet model.SimplePet
	...
	t.Run("Account", func(t *testing.T) {
		...
		if user.Account.Number != "" {
			if !user.Account.UserID.Valid {
				...
				var account model.SimpleAccount
	...
	t.Run("Pets", func(t *testing.T) {
		...
		for idx, pet := range user.Pets {
			if pet == nil || expect.Pets[idx] == nil {
				...
				CheckSimplePet(t, *pet, *expect.Pets[idx])
			}
		}
	})
...
func CheckUser(t *testing.T, user model.User, expect model.SimpleUser) {
	if user.ID != 0 {
		var newUser model.SimpleUser
	...
	t.Run("Account", func(t *testing.T) {
		...
		if user.Account.Number != "" {
			if !user.Account.UserID.Valid {
				...
				var account model.SimpleAccount
				DB.Model(&model.Account{}).First(&account, "user_id = ?", user.ID)
				AssertObjEqual(t, account, user.Account, "ID", "UserID", "Number")
			}
		}
	})
